...
for k=stats.kIni:stats.kEnd, 
  ...
  if showFlag
    ...
    S_show=reshape( showNormalize(S, vminShow, vmaxShow, grayFlag), [Nrows, Ncols, nDims]);   
    
    S_show=imresize(S_show,2.1);%comentar
    M=reshape( showNormalize(curFrame), [Nrows, Ncols, nDims]);
   
    %%% ----detección personas----
     peopleDetector = vision.PeopleDetector('ClassificationModel','UprightPeople_96x48');
     peopleDetector.ClassificationThreshold =2.6;%2.7
     peopleDetector.MergeDetections=1;

     peopleDetector.MinSize=[96,48];
     peopleDetector.MaxSize=[250,250];%200,200
% 
     [bboxes,scores] = peopleDetector(S_show);
% 
%     
     if ~isempty(scores)
        S_show = insertObjectAnnotation(S_show,'rectangle',bboxes,scores);
     end  
%     
    figure(4); imagesc(S_show); colormap gray;
    figure(5); imagesc(M); colormap gray;    
    drawnow;
